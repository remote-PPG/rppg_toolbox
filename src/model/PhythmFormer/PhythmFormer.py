...
class Fusion_Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        B, T, C, H, W = x.shape
        x1 = torch.cat([x[:,:1,:,:,:],x[:,:1,:,:,:],x[:,:T-2,:,:,:]],1)
        x2 = torch.cat([x[:,:1,:,:,:],x[:,:T-1,:,:,:]],1)
        x3 = x
        x4 = torch.cat([x[:,1:,:,:,:],x[:,T-1:,:,:,:]],1)
        x5 = torch.cat([x[:,2:,:,:,:],x[:,T-1:,:,:,:],x[:,T-1:,:,:,:]],1)
        x_diff = self.stem12(torch.cat([x2-x1,x3-x2,x4-x3,x5-x4],2).view(B * T, 12, H, W))
        x3 = x3.contiguous().view(B * T, C, H, W)
        x = self.stem11(x3)
...
class PhythmFormer(nn.Module):
    ...
    def forward(self, x):
        B, T, C, H, W = x.shape
        x = self.Fusion_Stem(x)    #[B*T 64 H/4 W/4]
        x = x.view(B,T,64,H//4,W//4).permute(0,2,1,3,4)
        x = self.patch_embedding(x)    #[B 64 T 8 8]
        for i in range(3):
            x = self.stages[i](x)    #[B 64 T 8 8]
        features_last = torch.mean(x,3)    #[B, 64, T, 8]  
        features_last = torch.mean(features_last,3)    #[B, 64, T]  
        rPPG = self.ConvBlockLast(features_last)    #[B, 1, T]
        rPPG = rPPG.squeeze(1)
        # pred_ppg = (rPPG-torch.mean(rPPG, axis=-1).view(-1, 1))/torch.std(rPPG, axis=-1).view(-1, 1)
        return rPPG 