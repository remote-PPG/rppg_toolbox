...
class DeepPhys(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        original 2d model
        """
        B,_,T,C,W,H = inputs.shape
        inputs = inputs.permute(1, 0, 2, 3, 4, 5)
        # 输出：B,AM,T,C,W,H => AM,B,T,C,W,H
        appearance_input = inputs[0]
        # 输出：B,T,C,W,H
        motion_input = inputs[1]
        # 输出：B,T,C,W,H
        appearance_input = appearance_input.contiguous()
        appearance_input = appearance_input.view(B*T,C,W,H)
        # 输出：B*T,C,W,H
        motion_input = motion_input.contiguous()
        motion_input = motion_input.view(B*T,C,W,H)
        # 输出：B*C,T,W,H
